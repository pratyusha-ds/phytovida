...
export async function analyzePlant(file: File): Promise<DiagnosisResult> {
  ...
  if (res.status === 401) throw new Error('Invalid API key. Check VITE_PLANT_ID_API_KEY.')
  if (res.status === 402) throw new Error('No credits remaining on your Plant.id plan.')
  if (res.status === 429) throw new Error('Daily request limit reached. Try again tomorrow.')
  if (!res.ok) {
    const text = await res.text().catch(() => '')
    throw new Error(`Plant.id API error ${res.status}: ${text || 'request failed'}`)
  }
 
  // Plant.id returns remaining quota in this response header
  const remainingFromHeader = Number(res.headers.get('x-limit-requests-remaining') ?? 0)
